      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-6 py-16 text-center">
          <p className="text-sm text-slate-500">
            {polls.length === 0 ? (
              <>No polls yet. <Link href="/dashboard/create" className="text-violet-600 hover:underline">Create your first vote</Link>.</>
            ) : (
          ...
          {filtered.map((poll) => {
            ...
            return (
              <Link
                ...
                className="group relative flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-violet-300 hover:shadow-md"
              >
                {/* accent bar */}
                <div className={`absolute left-0 top-4 bottom-4 w-1 rounded-full ${
                  poll.status === "active" ? "bg-violet-500" :
                  poll.status === "ended" ? "bg-slate-300" : "bg-amber-400"
                }`} />

                <div className="pl-3">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-sm font-semibold text-slate-900 group-hover:text-violet-600 leading-snug">
                      {poll.title}
                    </h3>
